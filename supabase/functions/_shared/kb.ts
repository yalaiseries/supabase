export const websiteKb = String.raw`# AI (Hackathon) Series — Agentic AI Learning Hub (2026)

## Objectives
- Upskill local and international young Architects, practitioners and firms in using AI and digital technologies to develop practical data-driven solutions that enhance creativity and productivity.
- Foster knowledge-sharing, innovation and collaboration within the Architecture and Built Environment community through ground-up initiatives.

## Target Outcomes
- Enable participants to collaboratively learn, develop, and share AI-assisted approaches and applications, creating practical and innovative solutions together with the community of practice (CoPr).
- This approach fosters collective initiatives and promotes scalable, collaborative outcomes, enabling participants to learn and build simultaneously.
- All online sessions will be recorded and shared within the CoPr, serving as resources for learning, exploring, and deploying AI-assisted applications. ArchiFest 2026 will showcase top entries.

## Theme
- Agentic AI — Collaborate and Innovate

## Topics
Open to practical topics, for example:
- Design
- Checks
- Manage
- Contract
- Fabrication
- Construction and O&M
- Others

## Target Participants
Local and/or international young architects, practitioners, firms, and tech partners looking to enter the built environment can participate as individuals or self-formed groups.

## Judging Criteria
- Practicality & Impact – A well-defined problem with a feasible solution that addresses real-world needs.
- Collaboration & Innovation – Meaningful collaboration and a clear, novel application of AI to improve outcomes.
- Documentation & Implementation – Clear, reproducible documentation of approach, workflow, and results.

## Benefits
Participants may benefit from the event in:
1. Problem-Solving: Explore AI-assisted strategies to solve practical issues within a self-defined time
2. Upskilling: Learn AI with a community of practice at a self-driven pace
3. Networking: Connect with industry leaders and peers through the event
4. Recognition: Celebrate top projects with prizes at ArchiFest 2026 in July
5. Certification: Earn e-certification upon completion

## AI (Hackathon) Cash Prizes (to be confirmed)
- First Prize: $3000 (1)
- Second Prize: $2000 (1)
- Third Prize: $1000 (1)
- Innovation Awards: $500 (5)

## Registration Links
- AI (Hackathon) Series (2026 AI Collaboration): https://forms.gle/rdZZzCxTVhAAzvCP7
- Discussion & Sharing Sessions: to be updated

## Target Deliverables
The deliverables include a two-page write-up and a max. 10-slide presentation based on the provided guided structure.

Submission dates:
- 18 Mar: Prelim submission
- 15 Apr: Midterm submission
- 13 May: Final submission

Shortlisted projects proceed to a presentation session on 20 May.

## Timeline
- 5 Jan: Interest Registration and Topic Survey
- 11 Feb: Announcement in website
- 25 Feb: Physical Session (Update and Grouping; Sharing by Ar SIM Quentin)
- 11 Mar: Hybrid Session (Discussion; Sharing by Mr Samuel OOI)
- 18 Mar 11:59pm: Problem Statement Submission
- 25 Mar: Hybrid Session (Discussion; Sharing by Mr Jason LI)
- 8 Apr: Hybrid Session (Discussion; Sharing by Mr Claurence LEE)
- 15 Apr 11:59pm: Solution Submission
- 22 Apr: Hybrid Session (Update; Sharing by Mr Benard LEONG; Sharing by Ar PAN Yicheng)
- 6 May: Hybrid Session (Optional)
- 13 May 11:59pm: Final Submission
- 20 May: Hybrid Session (Presentation & Assessment)
- 24 Jul: Physical Session (Forum & Ceremony at ArchiFest 2026)

## Venue
SIA Theatrette, 3rd Level, 79 Neil Road, Singapore 088904

## Zoom Link
https://us02web.zoom.us/j/82756306315?pwd=VDzmhi1Mp63faMHoSaA1uqBa3QTR6O.1

Meeting ID: 827 5630 6315

Passcode: 529788

## BOA-SIA CPD Points
BOA-SIA CPD points are awarded upon recorded attendance at sessions shown in the timeline:
- 3 sessions: 1 point
- 5 sessions: 2 points
- 7/8 sessions: 3 points

## Email Contact
- youth@sia.org.sg
- yal.aiseries@gmail.com

## Note & Disclaimer
The scope, deliverables, timelines, and terms of the AI (Hackathon) Series (2026 AI Collaboration) may change at the organisers’ discretion.
`;
