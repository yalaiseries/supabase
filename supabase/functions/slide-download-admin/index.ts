...
async function loadDownloadEvents(input: {
  req: Request;
  supabaseUrl: string;
  serviceRoleKey: string;
}): Promise<
...
> {
  ...
  const slideId = String(requestUrl.searchParams.get('slide_id') || '').trim();
  const from = String(requestUrl.searchParams.get('from') || '').trim();
  const to = String(requestUrl.searchParams.get('to') || '').trim();

  const url = new URL(`${input.supabaseUrl}/rest/v1/slide_download_events`);
  url.searchParams.set('select', 'id,email,slide_id,slide_url,event_at,user_agent,referrer,request_method');
  ...
  if (slideId) {
    url.searchParams.set('slide_id', `eq.${slideId}`);
    filters.slide_id = slideId;
  }
  ...
  if (!resp.ok) {
    ...
    return { ok: false, status: 500, error: text || 'Failed to load download events.' };
  }
...
serve(async (req: Request) => {
  ...
  const events = await loadDownloadEvents({ req, supabaseUrl, serviceRoleKey });
  if (!events.ok) return json({ error: events.error }, events.status);
